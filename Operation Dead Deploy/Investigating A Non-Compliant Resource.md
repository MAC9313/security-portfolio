...
## Scenario
A non-compliant resource group (rg) was created under the Mad Hat Labs subscription. The objective is to find out who is responsible for creation, what was deployed within the rg, when the rg was created, why and how the non-compliant rg was allowed to be deployed in the first place.
## Environment
In a live multi-user Azure training tenant, maintaining reader access over the Mad Hat Subscription. RG's use a typical naming convention that start with rg, anything that does not utilize this naming convention is a finding and should be investigated further.  
...
## What I learned
- The principle of least privilege is not a recommendation, it is necessary to prevent junior operatives from creating non-compliant resources. In this scenario, it was simple naming enforcement that failed. In the next incident, maybe a resource is assigned a public ip address that gives an adversary their initial foothold in the network. 
